...
export default function YouTubePlaylistPlayer({ playlistId, videoId, sourceUrl, playlistTitle, thumbnailUrl, lyricsUrl, userName, dateLabel, canDelete, onDelete, deleting, deleteLabel = "Borrar publicación", canEditAlbum, onEditAlbum, editAlbumLabel = "Editar álbum" }) {
  ...
  const handleMessage = (event) => {
    try {
      ...
      if (data.type === "playerState" && Array.isArray(data.ids)) {
        setVideoIds(data.ids);
        setActiveIndex(data.index || 0);
        setCurrentTime(Number(data.time) || 0);
      }
    } catch {}
  };
        ...
        <SafeAreaView style={styles.detailRoot}>
          ...
          <ScrollView style={styles.detailScroll} contentContainerStyle={styles.detailContent}>
            ...
            <View style={styles.footer}>
              ...
              <Pressable onPress={() => sourceUrl && Linking.openURL(sourceUrl)} disabled={!sourceUrl} style={({ pressed }) => [styles.openButton, pressed && styles.pressed]}><Text style={styles.openButtonText}>Abrir en YouTube</Text></Pressable>
            </View>
          </ScrollView>
        </SafeAreaView>
      </Modal>
